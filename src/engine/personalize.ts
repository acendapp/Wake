...
4. Choose and order moves to fit THIS person — their intent, fitness, friction point, goals, constraints, and how recent mornings actually went (the reflections). Lead with the single highest-leverage move for them; that is "leadSlug". Freshness is a hard rule: NEVER choose as "leadSlug" a slug listed in "recentFocalPoints", nor another variant of the same underlying goal — those led recently and are on cooldown. (They may still appear as non-lead moves.)
